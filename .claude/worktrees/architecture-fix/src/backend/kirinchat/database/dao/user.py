...
class UserDao:
    ...
    @classmethod
    async def get_user(cls, user_id: str) -> UserTable | None:
    ...
    @classmethod
    async def get_user_by_ids(cls, user_ids: List[str]) -> List[UserTable] | None:
        async with async_session_getter() as session:
    ...
    @classmethod
    async def get_user_by_username(cls, user_name: str) -> UserTable | None:
        async with async_session_getter() as session:
    ...
    @classmethod
    async def update_user(cls, user_id: str, user_name: str, user_email: str, user_password: str) :
        async with async_session_getter() as session:
    ...
    @classmethod
    async def filter_users(cls, user_ids: List[str], keyword: str = None, page: int = 0, limit: int = 0):
        ...
        async with async_session_getter() as session:
            ...
            return users_result.all(), count_result.one()

    @classmethod
    async def get_unique_user_by_name(cls, user_name: str) -> UserTable | None:
        async with async_session_getter() as session:
            statement = select(UserTable).where(UserTable.user_name == user_name)
            result = await session.exec(statement)
            return result.first()
    ...
    @classmethod
    async def create_user(cls, user_id: str, user_name: str, user_email: str, user_password: str):
        async with async_session_getter() as session:
    ...
    @classmethod
    async def add_user_and_default_role(cls, user_name: str, user_email: str, user_password: str, user_avatar: str):
        """
        新增用户，并添加默认角色
        用户的ID以此递增
        """
        user_number = len(await cls.get_user_number()) + 1
        async with async_session_getter() as session:
    ...
    @classmethod
    async def add_user_and_admin_role(cls, user_id: str, user_name: str,
    ...
    @classmethod
    async def get_all_users(cls, page: int = 0, limit: int = 0) -> List[UserTable]:
        """
        分页获取所有用户
        """
        statement = select(UserTable)
    ...
    @classmethod
    async def get_visible_users(cls):
        async with async_session_getter() as session:
    ...
    @classmethod
    async def get_user_number(cls) -> int:
        async with async_session_getter() as session:
            statement = select(UserTable)
            result = await session.exec(statement)
            return result.all()
    ...
    @classmethod
    async def update_user_info(cls, user_id, user_avatar, user_description):
        async with async_session_getter() as session:
            update_values = {}
            if user_avatar:
