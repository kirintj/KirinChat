...
class MemoryHistoryDao:
    ...
    @classmethod
    async def get_history(cls, memory_id: str) -> List[Dict[str, Any]]:
        """获取指定memory_id的历史记录"""
        try:
            async with async_session_getter() as session:
    ...
    @classmethod
    async def get_all_history(cls) -> List[Dict[str, Any]]:
        """获取所有历史记录"""
        try:
            async with async_session_getter() as session:
    ...
    @classmethod
    async def delete_history_by_memory_id(cls, memory_id: str) -> None:
    ...
    @classmethod
    async def delete_history_by_id(cls, history_id: str) -> None:
        """删除指定ID的历史记录"""
        try:
            async with async_session_getter() as session:
                sql = delete(MemoryHistoryTable).where(
    ...
    @classmethod
    async def mark_as_deleted(cls, history_id: str) -> None:
        """标记历史记录为已删除"""
        await cls.update_history_record(history_id, is_deleted=True)

    @classmethod
    async def get_history_by_event(cls, event: str) -> List[Dict[str, Any]]:
        """根据事件类型获取历史记录"""
        try:
            async with async_session_getter() as session:
                sql = select(MemoryHistoryTable).where(
    ...
    @classmethod
    async def get_history_by_actor(cls, actor_id: str) -> List[Dict[str, Any]]:
        """根据操作者ID获取历史记录"""
        try:
            async with async_session_getter() as session:
                sql = select(MemoryHistoryTable).where(
