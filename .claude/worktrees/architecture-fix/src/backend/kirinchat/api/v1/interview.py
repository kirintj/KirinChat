...
from kirinchat.middleware.rate_limiter import limiter
...
async def download_evaluation_pdf(request: Request, evaluation_id: str, login_user: UserPayload = Depends(get_login_user)):
    """下载面试评估报告 PDF。"""
    try:
        report = await EvaluationService.get_report_by_id(evaluation_id)
        if not report:
